...
const Register = () => {
  const {httpRequest, saveToken} =  AxiosAuthClient()
  ...
  const handleSubmit = (e) => {
    ...
    httpRequest.post('/api/v1/register', {name: name, email : email, password : password}).then((response) =>{
      toast.success('Registration successful');
      saveToken(response.data.token, rememberMe)
    }).catch((error) => {
      toast.success('Registration unsuccessful');
    });
  };
  return (
        ...
        <div className='row'>
            <div className='col-md-8 col-sm-12 side-text'>
              <div className="col-md-9">
                ...
                You can  <Link to={'/login'} style={{textDecoration: "none", color: "#4D47C3", fontWeight:"bold"}}>login here</Link>
              </div>
